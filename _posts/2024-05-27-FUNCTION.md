...
| DATE_FORMAT<br />(날짜, 날짜 포멧)                           | 날짜의 형식을 지정한 포멧으로 변경<br />포멧은 ''안에 입력 되어야 하며 대소문자를 구분.<br />%Y : 네 자리수 연도, %y : 년도 뒤 두 자리만 표시  <br />%M : 월을 영어 표시, %m : 두 자리수 월<br />%D : 일 뒤에 th 표시, %d : 두 자리수 일 | DATE_FORMAT<br />(CURRENT_DATE(), '%Y/%m/%d') | 2024/05/28          |
