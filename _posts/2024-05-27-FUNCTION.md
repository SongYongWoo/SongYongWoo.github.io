...
| DATE_FORMAT(날짜, 날짜 포멧)                                 | 날짜의 형식을 지정한 포멧으로 변경<br />포멧은 ''안에 입력 되어야 하며 대소문자를 구분.<br />%Y : 네 자리수 연도, %y : 년도 뒤 두 자리만 표시  <br />%M : 월을 영어 표시, %m : 두 자리수 월<br />%D : 일 뒤에 th 표시, %d : 두 자리수 일 | DATE_FORMAT<br />(CURRENT_DATE(), '%Y/%m/%d') | 2024/05/28          |
| STR_TO_DATE(문자, 날짜 포멧)                                 | 문자로 표시된 날짜를 날짜 데이터로 변경<br />문자로 표시된 날짜와 날짜 포멧은 동일해야함. | STR_TO_DATE<br />('20200528','%Y%m%d')        | 20200528            |
| DATEDIFF(날짜1, 날짜2)                                       | 두 날짜 차이의 일 수를 계산<br />날짜1- 날짜2                | DATEDIFF<br />(CURRENT_DATE(),order_date)     | 3                   |
